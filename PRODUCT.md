...
- **Raff (builder).** Phone, between calls on site or in the ute. Opens on My items (his waiting-on items): dated actions to book, order or chase, ticked off as they're done. Also sees jobs (with the job switcher and each job's Shipments tab), photos and notes; no setup.
...
- Overview (home for partners and admin, jobs index for everyone): one card per job showing only the job's name, its stage progress bar and its overdue count; no next steps, waiting-on items or freshness (the job page holds the detail). No screen shows a finish date or money; the calculator stays underneath only to produce act-by and needed-by dates.
- Waiting on is its own bottom tab: one list, the same on phone and desktop, grouped Overdue (exactly the Overview's overdue count), This week, Later. Each row has the title, job, who it's waiting on, one date phrase, a Call button (a tel: link to the trade, or the person it's with, when there's a number) and its quick actions (Set date, the next status). Filters are Everyone | Mine and a job menu. There is no Call mode; old `?mode=call` and `#/calls` links show the list. The bell (notifications) sits top right.
